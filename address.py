...
class Address(Model):
    def __init__(self):
        ...
        #self.con.close()
    def getall(self):
        self.cur.execute("select * from address order by name")

        row=self.cur.fetchall()
        return row
    def getalladdress(self):
        self.cur.execute("select *, (name || ' &lt;' || email || '&gt;') myaddress from address order by name")
    ...
    def create(self,params):
        ...
        myid=None
        try:
          self.cur.execute("insert into address (name,email,user_id) values (:name,:email,:user_id)",myhash)
          self.con.commit()
